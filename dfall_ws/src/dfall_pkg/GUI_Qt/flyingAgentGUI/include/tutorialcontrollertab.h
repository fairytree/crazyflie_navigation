...
#ifndef TUTORIALCONTROLLERTAB_H
#define TUTORIALCONTROLLERTAB_H
...
namespace Ui {
class TutorialControllerTab;
}

class TutorialControllerTab : public QWidget
{
    Q_OBJECT

public:
    explicit TutorialControllerTab(QWidget *parent = 0);
    ~TutorialControllerTab();
...
private:
    ...
    // SUBSCRIBER
    // > For being notified when the optimization setup status is changed
    ros::Subscriber optimizationSetupStatusChangedSubscriber;
    ...
    // For receiving message that the optimization setup status was changed
    void optimizationSetupStatusChangedCallback(const dfall_pkg::SetpointWithHeader& newSetpoint);

    // Publish a message when a custom button is pressed
    void publish_custom_button_command(int button_index);
